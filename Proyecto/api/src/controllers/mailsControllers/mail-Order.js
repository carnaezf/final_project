...
const mailOrder = async (  ) => {
    const {idOrder, paymentMethod, product, quantity, totalMount, status} = Order
    const { name, sellingPrice, image } = Product
    ...
    //     status } = req.body;
        ...
        const to = User.email
        const subject = `¡Thanks for your purchase ${User.name} ${User.lastName}!`
